  ...
  return (
    <main className="max-w-4xl mx-auto pt-10">
      <section className="flex flex-col gap-8 px-4">
        <header className="flex justify-between flex-wrap items-center gap-y-4">
        ...
        {projects.data.length > 0 ? (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(330px,1fr))] gap-4">
            {projects.data.map(({ description, name, id }) => (
              <article
                key={id}
                className="relative bg-card py-6 px-8 rounded-md border hover:border-primary transition-all"
              >
                <h3 className="font-semibold">{name}</h3>
                <p className="text-muted-foreground">{description}</p>
                <Link href={`${params.slug}/${name}`} className="absolute block inset-0 z-10" />
              </article>
            ))}
          </div>
        ) : (
          <article className="mt-40 flex justify-center items-center flex-col gap-y-6">
